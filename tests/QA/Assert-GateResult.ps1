<#
    ...
    Run:  pwsh -File ./tests/QA/Assert-GateResult.ps1 -ResultPath output/testResults/NUnitXml_*.xml -MinimumTests 500
#>
[CmdletBinding()]
param(
...
if ($null -eq $topSuite) {
    ...
    if ($overallResult -notin @('Success', 'Passed')) {
...
Write-Host "GATE PASSED: $total tests, $failures failed, $errorsRaw errors - result Passed, total >= $MinimumTests." -ForegroundColor Green
exit 0
