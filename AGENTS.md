...
**Current release status:** GraphKit `0.2.2` is the immutable stable package on PSGallery. Current integrated source is versioned `0.3.0` and prepares an unpublished candidate; local package/test proof is not publication or exact-SHA remote CI. Phases 1-5 are implemented; live verification remains recorded separately per auth mode, descriptor, and operation because implementation is not evidence of service behavior.
...
**Remote CI contract.** `.github/workflows/ci.yml` runs PowerShell 7.4 and 7.6 across Windows, Ubuntu, and macOS. A source revision is CI-verified only when all six matrix jobs pass for that exact SHA; workflow existence or an older successful run is not evidence. Locally, 772 deterministic tests must pass under `./build.ps1 -Tasks test`, with zero failures, errors, or skips, and `tests/QA/Assert-GateResult.ps1` enforces the same minimum-count floor used by CI and package verification.
