...
Describe 'Test workflow release-proof generation' {
    ...
    It 'finalize atomically emits one proof and preserves the candidate when replacement fails' {
        ...
        $proof.testRun.summary.total | Should -Be 1472
...
"@
    ...
    It 'both publisher scripts switch to verifier-owned package snapshots' {
        ...
        $packageCopy = $privatePublisher.IndexOf(
            'Copy-Item -LiteralPath $package.FullName -Destination $target -Force',
            [StringComparison]::Ordinal)
        $proofCopy | Should -BeGreaterOrEqual 0
        $packageCopy | Should -BeGreaterThan $proofCopy -Because 'proof publication must finish before package discoverability'
    }
}
