...
function Get-GraphKitWorktreeEntry { param([string] $Root, [byte[]] $RawPath, [Text.UTF8Encoding] $Utf8, [AllowNull()][string] $IndexMode)
    $relative = Get-GraphKitRelativePath $RawPath $Utf8
    try { $capture = [GraphKit.R8.SourceCapture]::Capture($Root, $relative) }
    catch {
    ...
    $mode = [GraphKit.R8.SourceCapture]::ResolveEffectiveGitMode($capture.Mode, $capture.HasExecutableMode, $IndexMode)
...
function Get-GraphKitR8SourceState { param([string] $Root)
    ...
    $utf8=[Text.UTF8Encoding]::new($false,$true); Initialize-GraphKitSourceCapture
